describe("FileUtils", () => {
  ...
  describe("getExcelReadingMethod", () => {
    test("should return arrayBuffer for binary formats", () => {
      ...
      expect(FileUtils.getExcelReadingMethod("workbook.xlsb")).toBe(
        "arrayBuffer",
      );
    ...
    test("should return binaryString for ZIP-based formats", () => {
      ...
      expect(FileUtils.getExcelReadingMethod("workbook.xlsm")).toBe(
        "binaryString",
      );
    ...
    test("should handle non-Excel files", () => {
      ...
      expect(FileUtils.getExcelReadingMethod("document.txt")).toBe(
        "binaryString",
      );
  ...
  describe("constants", () => {
    test("should provide file extension constants", () => {
      ...
      expect(constants.EXCEL_EXTENSIONS).toEqual([
        "xlsx",
        "xls",
        "xlsm",
        "xlsb",
    ...
    test("should return immutable constants", () => {
      ...
      expect(constants1.BINARY_FORMAT_EXTENSIONS).toEqual(
        constants2.BINARY_FORMAT_EXTENSIONS,
      );
    });
  });
  ...
  describe("integration tests", () => {
    test("should work together consistently", () => {
      ...
      testFiles.forEach((testFile) => {
        ...
        expect(FileUtils.getExcelReadingMethod(testFile.name)).toBe(
          testFile.method,
        );
